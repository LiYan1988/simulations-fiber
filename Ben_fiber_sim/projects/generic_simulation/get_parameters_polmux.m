function p = get_parameters_polmux(p)
...
p.modulation.format = 'DP-16-QAM'; p.bits_per_symbol = 4;
p.data_x = round(rand(p.bits_per_symbol, p.N_symb));
p.data_y = round(rand(p.bits_per_symbol, p.N_symb));
...
p.electric1.data   = p.data_x(1:2, :); % Data for electric source
p.electric1.levels = e_levels;
p.electric1.filter = e_filter;

%p.electric2.data   = p.data(2:2, :); % Data for electric source
p.electric2.data   = p.data_x(3:4, :); % Data for electric source
p.electric2.levels = e_levels;
p.electric2.filter = e_filter;

%p.electric3.data   = p.data(1:1, :); % Data for electric source
p.electric3.data   = p.data_y(1:2, :); % Data for electric source
p.electric3.levels = e_levels;
p.electric3.filter = e_filter;

%p.electric4.data   = p.data(2:2, :); % Data for electric source
p.electric4.data   = p.data_y(3:4, :); % Data for electric source
p.electric4.levels = e_levels;
p.electric4.filter = e_filter;

%% Transmission parameters
% An SMF
% Transmission distance [m]
p.smf.L = 20e3; % Transmission distance [m]
p.smf.D = 17; % Dispersion parameters [ps/nm/km]
% Group-velocity dispersion [s^2/m]
p.smf.beta2 = p.smf.D*p.conv.D_to_beta2;
% Dispersion slope (TODO: Need to check on this calculation)
p.smf.beta3 = 0;
% Attenuation [1/m]
...
% Signal power into fiber
p.signal.launch_power_dBm = 0; % Default channel launch power
