...
figure(1003); semilogy(power_sort,ber_sort(1,:),'-o',power_sort,ber_sort(2,:),'-o','LineWidth',2);title('BER vs Input Power');xlabel('Input Power per Channel [dBm]'); ylabel('BER]');legend('Pol X','Pol Y');  grid on; hold on;
figure(1004); plot(power_sort,snr_sort(1,:),'-o',power_sort,snr_sort(2,:),'-o','LineWidth',2);title('SNR vs Input Power');xlabel('Input Power per Channel [dBm]'); ylabel('SNR [dB]');legend('Pol X','Pol Y'); grid on; hold on;


%GM model 1 channel 2 pol
% Pow_GM=[-10:2:10];
% SER_GM=[15.66,17.65,19.61,21.46,22.91,23.31,21.63,18.57,14.81,10.88,6.89];
