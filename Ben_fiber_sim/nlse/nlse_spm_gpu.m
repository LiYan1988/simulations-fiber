...
write_log(p, now, sprintf('Starting NLSE split-step solver with %d steps (%.2f m per step)', n, delta_z));

u = gpuArray(u); % Loading to GPU memory
...
u = gather(u); % Retreiving from GPU memory
