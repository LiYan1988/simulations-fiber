...
%% Find existing files
resultsExisting = load('resultsNew_50GHz.mat');
resultsExisting = resultsExisting.resultsNew;
...
rowIdx = 1;
colIdx = 1;
resultsExistingTemp = [resultsExisting(:, 2), resultsExisting(:, 1)];
for n = 1:length(parameterArray)
    if ismember([parameterArray(1, n), parameterArray(3, n)], resultsExistingTemp, 'rows')
        continue
    end
