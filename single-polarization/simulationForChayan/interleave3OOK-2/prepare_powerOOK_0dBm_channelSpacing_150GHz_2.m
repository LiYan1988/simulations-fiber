...
%%
simulationName = strsplit(mfilename, '_');
simulationName = strjoin(simulationName(2:end), '_');
folderName = fullfile(pwd, sprintf('cluster_%s', simulationName));
...
%% Find existing files
resultsExisting = importfileResultsLevel1_150GHz('resultsLevel1_150GHz.csv');

%% Find parameters need to run on cluster
% array=( "one;1;a;r" "two;2;b;s" "three;3;c;t" )
% powerQAM, powerOOK, symbolRate, channelSpacing
vstr = cell(ceil(length(parameterArray)/jobPerNode), jobPerNode);
rowIdx = 1;
colIdx = 1;
resultsExistingTemp = [resultsExisting{:, 2}, resultsExisting{:, 13}];
for n = 1:length(parameterArray)
    if ismember([parameterArray(3, n), parameterArray(1, n)], resultsExistingTemp, 'rows')
        continue
    end
...
variableArray = variableArray(~cellfun('isempty',variableArray));
