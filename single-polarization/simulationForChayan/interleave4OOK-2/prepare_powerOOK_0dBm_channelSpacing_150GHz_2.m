...
%% Find existing files
resultExisting = readtable('resultsLevel1_150GHz.csv');
...
rowIdx = 1;
colIdx = 1;
resultExistingTemp = [resultExisting{:, 2}, resultExisting{:, 17}];
for n = 1:length(parameterArray)
    if ismember([parameterArray(3, n), parameterArray(1, n)], resultExistingTemp, 'rows')
        continue
    end
