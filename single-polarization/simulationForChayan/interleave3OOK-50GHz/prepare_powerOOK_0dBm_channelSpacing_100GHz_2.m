...
failedJob = [150, 171, 177, 187, 188, 189, 202, 203, 204, 205, 211];
vstr = vstr(failedJob, :);

variableArray = cell(size(vstr, 1), 2);
for n=1:size(variableArray, 1)
    variableArray{n, 1} = strcat(vstr{n, 1:8});
    variableArray{n, 2} = strcat(vstr{n, 9:16});
end

%% Prepare bash files
for n = 1:size(variableArray, 1)
    for m = 1:size(variableArray, 2)
        modifyBash(...
            fullfile(pwd, 'simulateScenario.sh'), ...
            fullfile(folderName, sprintf('simulateScenario%d-%d.sh', failedJob(n), m)), ...
            sprintf('simulateScenario%d-%d', failedJob(n), m), ...
            wallTime, ...
            variableArray{n});
    end
end
