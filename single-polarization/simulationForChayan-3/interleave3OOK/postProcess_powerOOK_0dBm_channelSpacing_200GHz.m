...
totalAchievableRate = sum(resultLevel1.achievableDataRate(:, [2, 4]), 2)/1e9;

results = [QAMSymbolRate, QAMPowerdBm, QAMSNRdB, OOKSNRdB, totalAchievableRate];
...
%%
fig = figure;
[M, c] = contour(meshXsym, meshYpower, meshAIR);
c.LineWidth = 2;
c.ShowText  = 'off';
clabel(M,c,'Interpreter', 'latex', 'LabelSpacing', 600)
set(gca, 'TickLabelInterpreter', 'latex')
...
savefig(fullfile(figureFolder, sprintf('AIR_%dGHz.fig', channelSpacing)))
print(fullfile(figureFolder, sprintf('AIR_%dGHz', channelSpacing)), '-dpng', '-r600')
print(fullfile(figureFolder, sprintf('AIR_%dGHz', channelSpacing)), '-dpdf', '-r600', '-bestfit')