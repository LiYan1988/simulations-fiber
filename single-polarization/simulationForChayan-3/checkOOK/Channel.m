classdef Channel < matlab.mixin.Copyable
    ...
    properties (GetAccess=public, SetAccess={?SinglePolarization})
        ...
        % for OOK 1 is preferred, for 16QAM 0.2 is preferred
        symbolInFir % number of symbols in FIR
        % FIR filter coefficients, a vector generated by gaussdesign or
        % rcosdesign
        fir
        % Optical filter bandwidth
        opticalFilterBandwidth
        
        % Detection scheme, only effective in OOK channel
        isHeterodyne
    ...
    methods
        function obj = Channel(varargin)
            ...
            addParameter(p, 'opticalFilterBandwidth', 50e9, @isnumeric);
            
            % Dectection scheme, only effective in OOK channel
            addParameter(p, 'isHeterodyne', false, @islogical);
            ...
            obj.opticalFilterBandwidth = p.Results.opticalFilterBandwidth;
            
            % Detection scheme, only effective in OOK channel
            obj.isHeterodyne = p.Results.isHeterodyne;
