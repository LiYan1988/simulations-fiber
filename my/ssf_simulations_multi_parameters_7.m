...
% Simulate varying OOK power, 16QAM power is -1dBm
...
power_step = -10:1:10;
param_mp = cell(length(power_step)); % [dBm], power of each channel

for k=1:length(power_step) % OOK power
    fprintf('(%d)', k)
    parfor m=1:length(power_step) % 16QAM power
        fprintf('(%d, %d)', k, m)
        param_temp = param;
        ...
        param_temp.power_channel_time((N-1)/2+1) = 10^(power_step(m)/10)/1e3; % [W]
...
%% Save results
save mp7_simulation_varying_power.mat

%% Plot results
load mp7_simulation_varying_power.mat
